...
def process_batch_file(file_path, session=None, debug=False, resume=True):
    ...
    # Create tracker instance (assuming ProgressTracker exists)
    ...
    try:
        ...
        with tqdm(total=total_rows, desc="Processing URLs") as pbar:
            ...
            if resume and tracker:
                processed = len(tracker.data['processed'])
            ...
            for batch in processor.process_batches(df.iterrows()):
                for index, row in batch:
                    url = row['Url']
                    if not url or pd.isna(url):
                        continue
                        
                    if resume and tracker and tracker.is_processed(url):
                        continue
                        
                    try:
                        if process_document(url, session=session, debug=debug):
                            if tracker:
                                tracker.mark_success(url, "Downloaded")
                            processed += 1
                        else:
                            if tracker:
...
def process_url_downloads(url, doc_links, row, config):
    ...
    try:
        ...
        for doc_link in doc_links:
            filename = f"{base_filename}.{'docx' if doc_link['type'] == 'doc' else 'pdf'}"
    ...
    try:
        for url, field_list, year in zip(urls, fields, years):
            try:
                ...
                doc_links = find_document_links(url, debug=session_args.get('debug', False), 
...
def process_excel_file(args):
    ...
    try:
        ...
        df['Ban hành'] = pd.to_datetime(df['Ban hành'], format='%d/%m/%Y', 
        ...
        # Process chunks
        completed = 0
        with ProcessPoolExecutor(max_workers=config.get('max_processes', 4)) as executor:
            futures = [
                executor.submit(process_url_chunk, 
